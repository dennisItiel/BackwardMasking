...
%     balanced_n = min(size(vertical, 1), size(horizontal, 1));
    balanced_n = min_of_all_trials;
    vertical = vertical(1:balanced_n,:,:);
    horizontal = horizontal(1:balanced_n,:,:);

    all_data{SOA_inx, 1} = vertical;
    all_data{SOA_inx, 2} = horizontal;
end
...
for fold=1:10
    ...
    all_test_vertical = cell(6,1);
    all_test_horizontal = cell(6,1);
    ...
    for SOA_inx = SOAs_to_use
        ...
        all_test_vertical{SOA_inx} = vertical(test_idx,:,:);
        all_test_horizontal{SOA_inx} = horizontal(test_idx,:,:);
    end
